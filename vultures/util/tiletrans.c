 ...
 ***********************************************************************/
int read_png_file(char * filename, png_byte *** image, unsigned long * width, unsigned long * height, png_color * palette, int * num_pal)
{
	...
	*image = malloc(*height * sizeof(png_byte*));
	...
	if (*num_pal) { /* in our paletted images we need to translate the "transparent" color to real transparency */
		unsigned long ** pxdata = (unsigned long **)*image;
		unsigned long transcolor = (0xff << 24) + (palette[0].blue << 16) + (palette[0].green << 8) + palette[0].red;
		int i, j;
		for (i = 0; i < *height; i ++) {
			for (j = 0; j < *width; j++)
			{
 ...
 ********************************************************************************/
int write_png_file(FILE *outfile, png_byte ** tile, int width, int height, png_color * palette, int num_pal)
{
	png_structp png_ptr;
	png_infop info_ptr;
 ...
 *********************************************************************************/
png_byte ** grab_tile(png_byte ** img, int img_w, int img_h, int * tile_x,
                      int * tile_y, int * tile_w, int * tile_h, int * hs_x, int * hs_y)
{
	...
	int hscolor = 0xffffa3ff;
	int bordercolor = 0xff972787;
