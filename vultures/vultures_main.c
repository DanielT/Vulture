        ...
        case NHW_MAP:
            if (u.uz.dlevel != 0)
            {
               ...
               /* Center the view on the hero, if necessary */
                ...
                /* draw the mouse too, so that the mouse doesn't vanish during travel */
                vultures_mouse_draw();
...
int vultures_get_ext_cmd(void)
{
    ...
    for (i = 0; extcmdlist[i].ef_txt != NULL; i++)
    {
        ...
        while (cur_accelerator != used_accels[i] && used_accels[i])
            i++;
...
void vultures_get_nh_event(void)
{
    ...
    if (flags.travel)
    {
        if ((lastpos[1].x != lastpos[0].x || lastpos[1].y != lastpos[0].y) &&
