...
void jtp_putstr(winid window, int attr, const char *str)
{
  ...
  if (tempwindow->wintype == NHW_MESSAGE)
  {
    ...
    if (str)
    {
      ...
      if (j)
      {
        ...
        tempwindow->rows[0] = malloc(sizeof(int) + (i-j+2)*sizeof(char));
  ...
  else if (tempwindow->wintype == NHW_MENU)
  {
    ...
    if (!tempwindow->menu)
    {
      tempwindow->menu = malloc(sizeof(jtp_menu));
...
int jtp_nh_poskey(int *x, int *y, int *mod)
{
  ...
  if (jtp_whatis_active)
  {
    ...
    while (!result_key)
    {
      ...
      jtp_get_mouse_input(jtp_mcursor[V_CURSOR_NORMAL], JTP_MBUTTON_LEFT);;
...
void jtp_print_glyph(winid window, XCHAR_P x, XCHAR_P y, int glyph)
{
    ...
    if (cansee(x,y))
    {
        ...
        if (glyph >= GLYPH_ZAP_OFF && glyph < GLYPH_WARNING_OFF)
            ...
            jtp_map_specialeff[y][x] = V_TILE_NONE;
        ...
        else if (jtp_map_specialeff[y][x] == V_TILE_NONE)
            jtp_map_mon[y][x] = V_TILE_NONE;
        ...
        if ((obj = vobj_at(x,y)) && !covers_objects(x,y))
        {
             ...
             * corpses... */
            jtp_map_obj[y][x] = jtp_object_to_tile(glyph_to_obj(obj_to_glyph( obj)), x, y);
        }
        /* just to make things intersting, the above does not handle thrown/kicked objects... */
        else if (glyph_is_object(glyph))
        {
            jtp_map_obj[y][x] = jtp_object_to_tile(glyph_to_obj(glyph), x, y);
        }
        else
            jtp_map_obj[y][x] = V_TILE_NONE;
        ...
        if (IS_FURNITURE(level.locations[x][y].typ))
        {
            ...
            jtp_map_back[y][x] = V_TILE_FLOOR_COBBLESTONE;
        }
        else if (JTP_EXTRA_FURNITURE(level.locations[x][y].typ))
        {
            ...
            jtp_map_furniture[y][x] = jtp_cmaptiles[glyph_to_cmap(back_to_glyph(x,y))];
            jtp_map_back[y][x] = V_TILE_FLOOR_ROUGH;
        }
        else
        {
            jtp_map_furniture[y][x] = V_TILE_NONE;
    ...
    else
    {
        ...
        if (glyph_is_monster(glyph))
        {
            ...
            if (jtp_map_back[y][x] == V_TILE_UNMAPPED_AREA && level.locations[x][y].typ != STONE)
                jtp_map_back[y][x] = V_TILE_FLOOR_NOT_VISIBLE;
        }
        ...
        {
            mapglyph(glyph, &character, &colour, &jtp_map_specialattr[y][x], x, y);
            jtp_map_mon[y][x] = V_TILE_INVISIBLE_MONSTER;
        }

        /* handle monsters you are warned of */
        else if (glyph_is_warning(glyph))
        {
            jtp_map_mon[y][x] = V_TILE_WARNLEV_1 + glyph_to_warning(glyph);
            if (jtp_map_back[y][x] == V_TILE_UNMAPPED_AREA && level.locations[x][y].typ != STONE)
                jtp_map_back[y][x] = V_TILE_FLOOR_NOT_VISIBLE;
        }
        ...
        else if (glyph_is_trap(glyph))
        {
            ...
            if (jtp_map_back[y][x] == V_TILE_UNMAPPED_AREA)
                jtp_map_back[y][x] = V_TILE_FLOOR_NOT_VISIBLE;	    
        }
        ...
        if (level.locations[x][y].seenv != 0 && jtp_map_back[y][x] == V_TILE_FLOOR_NOT_VISIBLE)
        {
            if (IS_FURNITURE(level.locations[x][y].typ))
            {
                jtp_map_furniture[y][x] = jtp_cmaptiles[glyph_to_cmap(back_to_glyph(x,y))];
         ...
         * converted via jtp_cmaptiles are currently V_TILE_NONE */
        if ( ((!level.locations[x][y].waslit) && jtp_map_back[y][x] == V_TILE_FLOOR_COBBLESTONE) ||
           (level.locations[x][y].typ == ROOM && jtp_map_back[y][x] == V_TILE_UNMAPPED_AREA &&
    ...
    {
        jtp_map_furniture[y][x] = V_TILE_HODBRIDGE;
        switch (level.locations[x][y].drawbridgemask & DB_UNDER)
        {
            case DB_MOAT:  jtp_map_back[y][x-1] = jtp_cmaptiles[S_pool]; break;
            case DB_LAVA:  jtp_map_back[y][x-1] = jtp_cmaptiles[S_lava]; break;
            case DB_ICE:   jtp_map_back[y][x-1] = jtp_cmaptiles[S_ice];  break;
            case DB_FLOOR: jtp_map_back[y][x-1] = jtp_cmaptiles[S_room]; break;
        }
    }
    ...
    {
        jtp_map_furniture[y][x] = V_TILE_VODBRIDGE;
        switch (level.locations[x][y].drawbridgemask & DB_UNDER)
        {
            case DB_MOAT:  jtp_map_back[y-1][x] = jtp_cmaptiles[S_pool]; break;
            case DB_LAVA:  jtp_map_back[y-1][x] = jtp_cmaptiles[S_lava]; break;
            case DB_ICE:   jtp_map_back[y-1][x] = jtp_cmaptiles[S_ice];  break;
            case DB_FLOOR: jtp_map_back[y-1][x] = jtp_cmaptiles[S_room]; break;
        }
    }
...
void jtp_display_file(const char *fname, BOOLEAN_P complain)
{
  ...
  while (dlb_fgets(tempbuffer, 1024, f))
  {
    ...
    textlines[nlines-1] = malloc(strlen(tempbuffer)+1);
...
winid jtp_create_nhwindow(int type)
{
  ...
  tempwindow = malloc(sizeof(jtp_window));
  if (!tempwindow)
  {
    OOM(1);
  }
  ...
  switch(type)
  {
    ...
    case NHW_BASE:
      ...
      tempwindow->rows = malloc(tempwindow->curs_rows*sizeof(int *));
      for (i = 0; i < tempwindow->curs_rows; i++)
        tempwindow->rows[i] = malloc(tempwindow->curs_cols*sizeof(int));
    ...
    case NHW_STATUS:
      ...
      tempwindow->rows = malloc(tempwindow->curs_rows*sizeof(char *));
      for (i = 0; i < tempwindow->curs_rows; i++)
      {
        tempwindow->rows[i] = malloc(tempwindow->curs_cols*sizeof(char));
	tempwindow->rows[i][0] = 0;
      }
      tempwindow->curs_x = 0;
      tempwindow->curs_y = 0;
    ...
    case NHW_TEXT:
      ...
      for (i = 0; i < tempwindow->curs_rows; i++)
      {
        ...
        for (j = 0; j < tempwindow->curs_cols; j++)
          tempwindow->rows[i][j] = V_TILE_NONE;
      }
...
)
{
  ...
  tempmenuitem = malloc(sizeof(jtp_menuitem));
  if (!tempmenuitem)
  {
    OOM(1);
  }
  ...
  tempmenuitem->text = malloc(strlen(tempbuffer)+1);
...
void jtp_end_menu(winid window, const char *prompt)
{
  ...
  if (prompt)
  {
    tempwindow->menu->prompt = malloc(strlen(prompt)+1);
  ...
  {
    tempbutton = malloc(sizeof(jtp_button));
    if (!tempbutton)
    {
      OOM(1);
    }

    tempbutton->text = malloc(strlen("Continue")+1);
    if (!tempbutton->text)
    {
      OOM(1);
    }
  ...
  else /* For non-textual (possibly selectable) content, add Accept and Cancel buttons */
  {
    ...
    tempbutton->id = 0;
    tempbutton->accelerator = '\033'; /* ESC */
    jtp_list_reset(tempwindow->buttons);
    jtp_list_add(tempwindow->buttons, tempbutton);
    ...
    tempbutton->text = malloc(strlen("Accept")+1);
    if (!tempbutton->text)
    {
      OOM(1);
    }
