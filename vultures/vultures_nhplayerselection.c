...
static int
vultures_player_selection_role (void) {
    winid window;
    anything identifier;
    menu_item *menu_list;
    ...
    for (chosen=0;roles[chosen].name.m||roles[chosen].name.f;chosen++) {
        ...
        if (ok_role(chosen,flags.initrace,flags.initgend,flags.initalign)) {
            ...
            if (flags.initgend==1&&roles[chosen].name.f) {
                ...
                jtp_add_menu ( window, NO_GLYPH, &identifier, thisch, 0, 0, roles[chosen].name.f, chosen==flags.initrole);
            } else {
                thisch = lowc(roles[chosen].name.m[0]);
                if (thisch == lastch) thisch = highc(thisch);
                jtp_add_menu ( window, NO_GLYPH, &identifier, thisch, 0, 0, roles[chosen].name.m, chosen==flags.initrole);
            }
            lastch=thisch;
        }
    }
    ...
    identifier.a_int=-1; jtp_add_menu ( window, NO_GLYPH, &identifier, '*', 0, 0, "Random", (flags.initrole<0));
    /* Don't use the rolename here, otherwise we might get "Pick a role for your wizard"
     * which is rather silly. */
    sprintf(selection,"Pick a role for your character");
    jtp_end_menu(window, selection);
    if (jtp_select_menu (window, PICK_ONE, &menu_list)==1) {
...
static int
vultures_player_selection_race (void) {
    winid window;
    anything identifier;
    menu_item *menu_list;
    int chosen,items,item=0;
    ...
    for (chosen=0;races[chosen].noun;chosen++) {
        ...
        if (ok_race(flags.initrole,chosen,flags.initgend,flags.initalign)) {
            ...
            if (thisch == lastch) thisch = highc(thisch);
            lastch=thisch;
            jtp_add_menu (window, NO_GLYPH, &identifier, thisch, 0, 0, races[chosen].noun, chosen==flags.initrace);
        }
    }
    ...
    identifier.a_int=-1; jtp_add_menu ( window, NO_GLYPH, &identifier, '*', 0, 0, "Random", (flags.initrace<0));
...
static int
vultures_player_selection_gender (void) {
    winid window;
    anything identifier;
    menu_item *menu_list;
    ...
    while (chosen<ROLE_GENDERS) {
        ...
        if (ok_gend(flags.initrole,flags.initrole,chosen,flags.initalign)) {
            ...
            if (thisch == lastch) thisch = highc(thisch);
            lastch=thisch;
            jtp_add_menu ( window, NO_GLYPH, &identifier, thisch, 0, 0, genders[chosen].adj, chosen==flags.initgend);
            items++;
            item=chosen;
        }
        chosen++;
    ...
    identifier.a_int=-1; jtp_add_menu ( window, NO_GLYPH, &identifier, '*', 0, 0, "Random", (flags.initgend<0));
...
static int
vultures_player_selection_alignment (void) {
    winid window;
    anything identifier;
    menu_item *menu_list;
    int chosen,items,item=0;
    ...
    while (chosen<ROLE_ALIGNS) {
        ...
        if (ok_align(flags.initrole,flags.initrace,flags.initgend,chosen)) {
            ...
            if (thisch == lastch) thisch = highc(thisch);
            lastch=thisch;
            jtp_add_menu ( window, NO_GLYPH, &identifier, thisch, 0, 0,
    ...
    identifier.a_int=-1; jtp_add_menu ( window, NO_GLYPH, &identifier, '*', 0, 0, "Random", (flags.initalign<0));
...
*/
void
vultures_player_selection (void)
{
#ifdef DEBUG
    ...
    switch (jtp_yn_function("Shall I pick a character's race, role, gender and alignment for you?","ynq",'y')) {
        case 'q':
            clearlocks();
            jtp_exit_nhwindows("Quit from character selection.");
