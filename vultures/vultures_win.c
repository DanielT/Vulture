...
int jtp_query_anykey(const char *ques)
{
    ...
    if (keycount)
    {
        ...
        /* the value 255 will take jtp_translate_key out of "counting" mode
         * and prevent following direction keys from going through untranslated */
        jtp_sdl_keybuf_add(255);
...
int jtp_get_menu_selection(jtp_window *menuwindow)
{
  ...
  while (selectedbutton < 0)
  {
    ...
    if ((selectedbutton < 0) && (!widget_found) && (tempmenuitem))
    {
        jtp_list_reset(menuwindow->menu->items);
        tempmenuitem = (jtp_menuitem *)jtp_list_current(menuwindow->menu->items);
    }
