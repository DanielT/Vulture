...
int vultures_get_input(int force_x, int force_y, const char *ques, char *input)
{
    ...
    subwin->h = vultures_get_lineheight(V_FONT_INPUT);
...
void vultures_event_dispatcher(void * result, int resulttype, struct window * topwin)
{
    ...
    point mouse, mouse_old;
    SDL_Event event;
    int hovertime = 0, hovertime_prev = 0;
    int redraw;
    ...
    {
        redraw = 0;
        mouse_old = vultures_get_mouse_pos();
    ...
    /* someone might change the screen while we're not looking, 
     * so first restore the current saved background image */
    vultures_mouse_restore_bg();
}
