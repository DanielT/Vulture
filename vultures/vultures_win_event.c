    ...
    else if (event->type == SDL_KEYDOWN)
    {
        ...
        switch (event->key.keysym.sym)
        {
            ...
            default:
                ...
                /* more translation (Stage 2: apply keymap) */
                key = vultures_translate_key(key);
        ...
        switch (key)
        {
            ...
            case MENU_SEARCH:
                ...
                if (vultures_get_input(-1, -1, "What are you looking for?", str_to_find) != -1)
                {
                    ...
                    while (winelem)
                    {
                        if (strstr(winelem->caption, str_to_find) && winelem->scrollable)
                        {
                ...
                if (action)
                {
                    ...
                    switch (action)
                    {
                        ...
                        case V_INVACTION_REMOVE:
                            ...
                            vultures_eventstack_add('a', -1,-1, V_RESPOND_ANY);
                            vultures_eventstack_add('A', -1, -1, V_RESPOND_POSKEY);
                            return V_EVENT_HANDLED_FINAL;
