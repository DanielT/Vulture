...
int jtp_cmd_is_repeated(int cmd)
{
    if (cmd == jtp_keymaps[JTP_NHCMD_REPEATED_COMMAND].nhkey_numpad)
    	return 1;
    	
    else if (cmd == jtp_keymaps[JTP_NHCMD_DROP].nhkey_numpad)
    	return 1;
...
int jtp_translate_key(int cmd_key)
{
  ...
  if ((iflags.num_pad) &&
      jtp_cmd_is_repeated(jtp_last_translated_key) &&
      isdigit(cmd_key))
  {
    i = cmd_key;
...
void jtp_read_key_configuration(char *key_config_file)
{
  ...
  if (!f)
  {
    /* use defaults, but log failure */
    jtp_write_log_message(JTP_LOG_DEBUG, __FILE__, __LINE__, "Failed to open key config files jtp_keys.txt\n");
    return;
  }
...
void jtp_set_nethack_keys(void)
{
  ...
  cmd_index = JTP_NHCMD_PSEUDO_BARRIER;
  jtp_keymaps[cmd_index].description = "pseudo:barrier";
  jtp_keymaps[cmd_index].nhkey = 255;
  jtp_keymaps[cmd_index].flags = 0;
