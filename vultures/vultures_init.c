  ...
  } else
  {
    ...
    } else
    {
      ...
      for (i = 0; i < nScenes; i++)
      {
        ...
        vultures_refresh_region(0, image->h, vultures_screen->w, vultures_screen->h);
...
void vultures_show_ending(struct window * win)
{
    int lines;
    point textpos;
    SDL_Surface *image;
    SDL_Event event;        /* we don't do anything with this, it's only here for _wait_input */
    ...
    while (subwin)
    {
        textpos.x = (vultures_screen->w - vultures_text_length(V_FONT_INTRO, subwin->caption))/2;
        vultures_put_text_shadow(V_FONT_INTRO, subwin->caption, vultures_screen, textpos.x,
    ...
    /* display for 60 secs or until we get a keypress */
    vultures_wait_input(&event, 60000);
...
int vultures_init_graphics(void)
{
    ...
    all_ok &= vultures_load_font(V_FONT_SMALL, V_FILENAME_FONT_SMALL, 0, 12);
    all_ok &= vultures_load_font(V_FONT_LARGE, V_FILENAME_FONT_LARGE, 0, 14);
...
void vultures_destroy_graphics(void)
{
    ...
    /* clear and free the message buffer */
    vultures_messages_destroy();
