...
static jtp_mouse_cursor *jtp_choose_target_cursor(int tgt_x, int tgt_y)
{
  ...
  if (jtp_map_obj[tgt_y][tgt_x] != JTP_TILE_NONE)  
    return(jtp_mcursor[JTP_CURSOR_TARGET_RED]);
  
  if (jtp_map_back[tgt_y][tgt_x] != JTP_TILE_NONE)  /* valid visible location  */
  {
    /* Closed doors get an 'open door' cursor */
    if ((jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_VDOOR_WOOD_CLOSED) ||
        (jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_HDOOR_WOOD_CLOSED))
      return(jtp_mcursor[JTP_CURSOR_OPENDOOR]);

    /* Stairs and ladders get a 'stairs' cursor */
    if ((jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_STAIRS_UP) ||
        (jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_STAIRS_DOWN) ||
        (jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_LADDER_UP) ||
        (jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_LADDER_DOWN))
      return(jtp_mcursor[JTP_CURSOR_STAIRS]);

    /* Fountains get a 'goblet' cursor */
    if (jtp_map_furniture[tgt_y][tgt_x] == JTP_TILE_FOUNTAIN)
      return(jtp_mcursor[JTP_CURSOR_GOBLET]);

    if (jtp_map_back[tgt_y][tgt_x] != JTP_TILE_WALL_GENERIC)
...
jtp_tilenumber jtp_object_to_tile(int obj_id)
{
    jtp_tilenumber tile_id;

    if (obj_id == CORPSE)
        return JTP_TILE_BONES;
    
    else if ((obj_id < 0) || (obj_id >= NUM_OBJECTS))
        return JTP_TILE_NONE;

    tile_id = jtp_objtiles[obj_id];
    ...
    if (!objects[obj_id].oc_name_known)
    {
        switch (tile_id)
        {
            ...
            case JTP_TILE_BAG_OF_HOLDING:
                return JTP_TILE_UNIDENTIFIED_BAG;
            ...
            case JTP_TILE_JADE:
                ...
                {
                    case CLR_RED:     return JTP_TILE_RED_GLASS;
                    case CLR_BLACK:   return JTP_TILE_BLACK_GLASS;
                    case CLR_GREEN:   return JTP_TILE_GREEN_GLASS;
                    case CLR_BROWN:   return JTP_TILE_BROWN_GLASS;
                    case CLR_MAGENTA: return JTP_TILE_VIOLET_GLASS;
                    case CLR_ORANGE:  return JTP_TILE_ORANGE_GLASS;
                    case CLR_YELLOW:  return JTP_TILE_YELLOW_GLASS;
                    case CLR_WHITE:   return JTP_TILE_WHITE_GLASS;
                    case CLR_BLUE:    return JTP_TILE_BLUE_GLASS;
                    default:          return JTP_TILE_BLACK_GLASS;
                }
                break;
            default: break; /* shut the compiler up, otherwise it will warn
             about 500 times: once for each unhandled type in the enum :P*/
        }
    }
...
jtp_tilenumber jtp_monster_to_tile(int mon_id, XCHAR_P x, XCHAR_P y)
{
    if (Invis && u.ux == x && u.uy == y)
        return JTP_TILE_PLAYER_INVIS;
	...
	{
            case A_LAWFUL:  return JTP_TILE_LAWFUL_PRIEST;
            case A_CHAOTIC: return JTP_TILE_CHAOTIC_PRIEST;
            case A_NEUTRAL: return JTP_TILE_NEUTRAL_PRIEST;
            default:        return JTP_TILE_UNALIGNED_PRIEST;
	}
    }
...
static int jtp_get_dropdown_command(int mx, int my, int tgtx, int tgty)
{
  ...
  else if (jtp_map_mon[tgty][tgtx] != JTP_TILE_NONE)
  {
    /* Add monster options: */
    if ((u.ux != tgtx) || (u.uy != tgty))
  ...
  if (jtp_map_obj[tgty][tgtx] != JTP_TILE_NONE)
  {
    /* Add object options: */
    mapglyph_offset =  jtp_map_obj[tgty][tgtx];
    switch(mapglyph_offset)
    {
      case JTP_TILE_LARGE_BOX: case JTP_TILE_ICE_BOX: case JTP_TILE_CHEST:
        if ((u.ux == tgtx) && (u.uy == tgty))
        {
        ...
        break;
      case JTP_TILE_SACK: case JTP_TILE_OILSKIN_SACK: case JTP_TILE_BAG_OF_HOLDING: case JTP_TILE_BAG_OF_TRICKS:
        if ((u.ux == tgtx) && (u.uy == tgty))
        {
          jtp_add_dropdown_action(&n_actions, &dropdown_actions, JTP_ACTION_LOOT, "Loot");
          jtp_add_dropdown_action(&n_actions, &dropdown_actions, JTP_ACTION_PICK_UP, "Pick up");
  ...
  if (jtp_map_furniture[tgty][tgtx] != JTP_TILE_NONE)
  {
    /* Add cmap options: */
    ...
    switch(mapglyph_offset)
    {
      ...
      case JTP_TILE_VDOOR_WOOD_OPEN: case JTP_TILE_HDOOR_WOOD_OPEN:
        if ((u.ux != tgtx) || (u.uy != tgty))
          if ((abs(u.ux-tgtx) <= 1) && (abs(u.uy-tgty) <= 1))
          {
      ...
      case JTP_TILE_VDOOR_WOOD_CLOSED: case JTP_TILE_HDOOR_WOOD_CLOSED:
        if ((u.ux != tgtx) || (u.uy != tgty))
          if ((abs(u.ux-tgtx) <= 1) && (abs(u.uy-tgty) <= 1))
          {
  ...
  if (jtp_map_trap[tgty][tgtx] != JTP_TILE_NONE)
  {
    jtp_add_dropdown_action(&n_actions, &dropdown_actions, JTP_ACTION_UNTRAP, "Untrap");
    if ((u.ux != tgtx) || (u.uy != tgty))
      if ((abs(u.ux-tgtx) <= 1) && (abs(u.uy-tgty) <= 1))
  ...
  if (jtp_map_back[tgty][tgtx] != JTP_TILE_NONE)
  {
    if ((u.ux != tgtx) || (u.uy != tgty))    
...
static int jtp_get_default_command(int tgtx, int tgty)
{
  ...
  if ((!selected_action) && (jtp_map_mon[tgty][tgtx] != JTP_TILE_NONE))
  {
    if ((u.ux != tgtx) || (u.uy != tgty))
  ...
  if ((!selected_action) && (jtp_map_obj[tgty][tgtx] != JTP_TILE_NONE))
  {
    mapglyph_offset = jtp_map_obj[tgty][tgtx];
    if ((u.ux == tgtx) && (u.uy == tgty))
      switch(mapglyph_offset)
  ...
  if ((!selected_action) && (jtp_map_furniture[tgty][tgtx] != JTP_TILE_NONE))
  {
    /* Add cmap options: */
    mapglyph_offset =  jtp_map_furniture[tgty][tgtx];
    if ((mapglyph_offset == JTP_TILE_STAIRS_DOWN) || (mapglyph_offset == JTP_TILE_LADDER_DOWN))
      if ((u.ux == tgtx) && (u.uy == tgty))
        selected_action = JTP_ACTION_GO_DOWN;
    if ((mapglyph_offset == JTP_TILE_STAIRS_UP) || (mapglyph_offset == JTP_TILE_LADDER_UP))
      if ((u.ux == tgtx) && (u.uy == tgty))
        selected_action = JTP_ACTION_GO_UP;
    if (mapglyph_offset == JTP_TILE_SINK)
      if ((u.ux != tgtx) || (u.uy != tgty))
        if ((abs(u.ux-tgtx) <= 1) && (abs(u.uy-tgty) <= 1))
          selected_action = JTP_ACTION_KICK;
    if (mapglyph_offset == JTP_TILE_FOUNTAIN)
      if ((u.ux == tgtx) && (u.uy == tgty))        
        selected_action = JTP_ACTION_DRINK;
    if ((mapglyph_offset == JTP_TILE_VDOOR_WOOD_CLOSED) || (mapglyph_offset == JTP_TILE_HDOOR_WOOD_CLOSED))
      if ((u.ux != tgtx) || (u.uy != tgty))
        if ((abs(u.ux-tgtx) <= 1) && (abs(u.uy-tgty) <= 1))
    ...
    if (inventory_page_changed)
    {
      ...
      while ((tempmenuitem) && (selectable_items < 25))
      {
        if (tempmenuitem->count != JTP_NOT_SELECTABLE)     
        {
          ...
          item_tile = jtp_object_to_tile(obj_id);
          
          if ((item_tile != JTP_TILE_NONE) && (jtp_tiles[item_tile]))
