...
jtp_tilenumber ** jtp_map_back = NULL;      /* background (floors, walls, pools, moats, ...) */
jtp_tilenumber ** jtp_map_furniture = NULL; /* furniture (stairs, altars, fountains, ...) */
jtp_tilenumber ** jtp_map_trap = NULL;      /* traps */
jtp_tilenumber ** jtp_map_obj = NULL;       /* topmost object */
jtp_tilenumber ** jtp_map_specialeff = NULL;   /* special effects: zap, engulf, explode */
jtp_tilenumber ** jtp_map_mon = NULL;       /* monster tile ID */
...
jtp_tilenumber jtp_map_swallow = JTP_TILE_NONE; /* the engulf tile, if any */
...
/* various tile arrays */
jtp_tile ** jtp_tiles = NULL;     /* Isometric map tile set */
...
static int jtp_back_to_map_symbol(int tile);
static int jtp_furniture_to_map_symbol(int tile);
...
)
{
    ...
    {
        if (!canseeself())
            jtp_tiles[JTP_TILE_PLAYER_INVIS] = jtp_make_invisible_player_tile(u.umonnum);
        else
            jtp_tiles[JTP_TILE_PLAYER_INVIS] = jtp_make_transparent_player_tile(u.umonnum);
    }
    ...
    if (u.uswallow && jtp_map_swallow != JTP_TILE_NONE)
    {
        ...
        jtp_map_swallow = JTP_TILE_NONE;

    for (i = 0; i < JTP_MAP_HEIGHT; i++)
    {
        for (j = 1; j < JTP_MAP_WIDTH; j++)
        {
            if (jtp_map_tile_is_dark[i][j])
                lightlevel = JTP_AMBIENT_LIGHT / 3;
            else
                lightlevel = jtp_map_light[i][j];

            if (jtp_map_back[i][j] == JTP_TILE_UNMAPPED_AREA)
            ...
            if (jtp_map_back[i][j] == JTP_TILE_WALL_GENERIC ||
            ...
            if (jtp_map_back[i][j] == JTP_TILE_FLOOR_WATER ||
                jtp_map_back[i][j] == JTP_TILE_FLOOR_ICE ||
                jtp_map_back[i][j] == JTP_TILE_FLOOR_LAVA ||
                jtp_map_back[i][j] == JTP_TILE_FLOOR_AIR)
            ...
            if ((tile_id >= JTP_TILE_FLOOR_COBBLESTONE) &&
                (tile_id <= JTP_TILE_FLOOR_DARK))		   
                cur_tile = jtp_get_floor_tile(tile_id, i, j);
            else if (tile_id == JTP_TILE_WALL_GENERIC)
            {
                /* Add the correct backgound color under wall tiles */
                cur_tile = jtp_tiles[JTP_TILE_UNMAPPED_AREA];
                lightlevel = JTP_AMBIENT_LIGHT;
            }
            else
...
static void jtp_init_floor_decors(int num_decors)
{
    ...
    int current_deco, old_deco, old_deco2;
    int deco_height;
    int deco_width;
    int xoffset, yoffset;
    ...
    /* do no more if we're on the rogue level, because decors there look dumb */
#ifdef REINCARNATION
    ...
    for (i = 0; i < num_decors; i++)
    {
        ...
        switch (roomno % 3)
        {
            case 0: current_deco = JTP_FLOOR_STYLE_CARPET; break;
            case 1: current_deco = JTP_FLOOR_STYLE_MURAL; break;
            case 2: current_deco = JTP_FLOOR_STYLE_MURAL_2; break;
        }

        deco_width = jtp_floors[current_deco].xspan;
        deco_height = jtp_floors[current_deco].yspan;
        ...
        while (retries-- && !placed)
        {
            ...
            while (ly <= rooms[roomno].hy && (old_deco = (jtp_map_deco[ly][lx] >> 4)) != 0)
            {
                while (lx <= rooms[roomno].hx && (old_deco2 = (jtp_map_deco[ly][lx] >> 4)) != 0)
                    lx += jtp_floors[old_deco2].xspan;
                ly += jtp_floors[old_deco].yspan;
                lx = rooms[roomno].lx;
            }
...
)
{
    ...
    if ((jtp_map_mon[tgt_y][tgt_x] != JTP_TILE_NONE) ||
        (jtp_map_obj[tgt_y][tgt_x] != JTP_TILE_NONE) ||
        (jtp_map_trap[tgt_y][tgt_x] != JTP_TILE_NONE) ||
        (jtp_map_furniture[tgt_y][tgt_x] != JTP_TILE_NONE))
    {
...
static void jtp_get_wall_tiles(int y, int x)
{
    ...
    /* x - 1: west wall  */
    if (x > 0 && jtp_map_back[y][x - 1] != JTP_TILE_WALL_GENERIC && jtp_map_back[y][x - 1] != JTP_TILE_UNMAPPED_AREA)
    {
        style = jtp_get_wall_decor(jtp_map_back[y][x - 1], y, x, y, x-1);
        jtp_maptile_wall[y][x].west = jtp_walls[style].west;
    }
    else
        jtp_maptile_wall[y][x].west = NULL;

    /* y - 1: north wall  */
    if (y > 0 && jtp_map_back[y - 1][x] != JTP_TILE_WALL_GENERIC && jtp_map_back[y - 1][x] != JTP_TILE_UNMAPPED_AREA)
    {
        style = jtp_get_wall_decor(jtp_map_back[y - 1][x], y, x, y - 1, x);
        jtp_maptile_wall[y][x].north = jtp_walls[style].north;
    }
    else
        jtp_maptile_wall[y][x].north = NULL;

    /* x + 1: east wall  */
    if (x < JTP_MAP_WIDTH - 1 && jtp_map_back[y][x + 1] != JTP_TILE_WALL_GENERIC &&
        jtp_map_back[y][x + 1] != JTP_TILE_UNMAPPED_AREA)
    {
        style = jtp_get_wall_decor(jtp_map_back[y][x + 1], y, x, y, x + 1);
        jtp_maptile_wall[y][x].east = jtp_walls[style].east;
    }
    else
        jtp_maptile_wall[y][x].east = NULL;

    /* y + 1: south wall  */
    if (y < JTP_MAP_HEIGHT - 1 && jtp_map_back[y + 1][x] != JTP_TILE_WALL_GENERIC &&
        jtp_map_back[y + 1][x] != JTP_TILE_UNMAPPED_AREA)
    {
        style = jtp_get_wall_decor(jtp_map_back[y + 1][x], y, x, y + 1, x);
        jtp_maptile_wall[y][x].south = jtp_walls[style].south;
    }
    else
...
static jtp_tile *jtp_get_floor_tile(int tile, int y, int x)
{
    ...
    if (deco && tile == JTP_TILE_FLOOR_COBBLESTONE)
    {
        style = (deco >> 4) - 1;
        deco_pos = (int)(deco & 0x0F);
        return jtp_floors[style].pattern[deco_pos];
    }

    style = jtp_get_floor_decor(tile, y, x);
    deco_pos = jtp_floors[style].xspan * (y % jtp_floors[style].yspan) + (x % jtp_floors[style].xspan);
    return jtp_floors[style].pattern[deco_pos];
}
...
static void jtp_get_floor_edges(int y, int x)
{
    ...
    int style = JTP_FLOOR_EDGE_STYLE_COBBLESTONE;
   ...
   /* Default: no floor edges around tile */
    ...
    if (x > 0 && tile != jtp_map_back[y][x-1] &&
       (tile + jtp_map_back[y][x-1]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE)) /* this prevents borders between water and ice*/
        jtp_maptile_floor_edge[y][x].west = jtp_floor_edges[style].west;

    if (y > 0 && tile != jtp_map_back[y-1][x] &&
       (tile + jtp_map_back[y-1][x]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].north = jtp_floor_edges[style].north;

    if (x < JTP_MAP_WIDTH - 1 && tile != jtp_map_back[y][x+1] &&
       (tile + jtp_map_back[y][x+1]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].east = jtp_floor_edges[style].east;

    if (y < JTP_MAP_HEIGHT - 1 && tile != jtp_map_back[y+1][x] &&
       (tile + jtp_map_back[y+1][x]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].south = jtp_floor_edges[style].south;

    /* "inward pointing" corners */
    if ((jtp_maptile_floor_edge[y][x].west) && (jtp_maptile_floor_edge[y][x].north))
        jtp_maptile_floor_edge[y][x].northwest = jtp_floor_edges[style].northwest;

    if ((jtp_maptile_floor_edge[y][x].west) && (jtp_maptile_floor_edge[y][x].south))
        jtp_maptile_floor_edge[y][x].southwest = jtp_floor_edges[style].southwest;

    if ((jtp_maptile_floor_edge[y][x].east) && (jtp_maptile_floor_edge[y][x].south))
        jtp_maptile_floor_edge[y][x].southeast = jtp_floor_edges[style].southeast;

    if ((jtp_maptile_floor_edge[y][x].east) && (jtp_maptile_floor_edge[y][x].north))
        jtp_maptile_floor_edge[y][x].northeast = jtp_floor_edges[style].northeast;

    /* "outward pointing" corners */
    if ((!jtp_maptile_floor_edge[y][x].west) && (!jtp_maptile_floor_edge[y][x].north) &&
        x > 0 && y > 0 && tile != jtp_map_back[y-1][x-1] &&
        (tile + jtp_map_back[y-1][x-1]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].northwest_bank = jtp_floor_edges[style].northwest_bank;

    if ((!jtp_maptile_floor_edge[y][x].east) && (!jtp_maptile_floor_edge[y][x].north) &&
        x < JTP_MAP_WIDTH-1 && y > 0 && tile != jtp_map_back[y-1][x+1] &&
        (tile + jtp_map_back[y-1][x+1]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].northeast_bank = jtp_floor_edges[style].northeast_bank;

    if ((!jtp_maptile_floor_edge[y][x].west) && (!jtp_maptile_floor_edge[y][x].south) &&
        x > 0 && y < JTP_MAP_HEIGHT-1 && tile != jtp_map_back[y+1][x-1] &&
        (tile + jtp_map_back[y+1][x-1]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].southwest_bank = jtp_floor_edges[style].southwest_bank;

    if ((!jtp_maptile_floor_edge[y][x].east) && (!jtp_maptile_floor_edge[y][x].south) &&
        x < JTP_MAP_WIDTH-1 && y < JTP_MAP_HEIGHT-1 && tile != jtp_map_back[y+1][x+1] &&
        (tile + jtp_map_back[y+1][x+1]) != (JTP_TILE_FLOOR_WATER + JTP_TILE_FLOOR_ICE))
        jtp_maptile_floor_edge[y][x].southeast_bank = jtp_floor_edges[style].southeast_bank;
}
...
)
{
    ...
    switch (floortype)
    {
        ...
        case JTP_TILE_FLOOR_NOT_VISIBLE:
        case JTP_TILE_FLOOR_COBBLESTONE:
        {
            roomid = jtp_get_room_index(floorx, floory);
            switch(roomid % 4)
            {
                case 0: return JTP_WALL_STYLE_STUCCO;
                case 1: return JTP_WALL_STYLE_BRICK + ((wally*wallx+wally+wallx)%5);
                case 2: return JTP_WALL_STYLE_VINE_COVERED;
                case 3: return JTP_WALL_STYLE_MARBLE;
            }
        }
        default:
...
)
{
    ...
    switch (floorstyle)
    {
        case JTP_TILE_FLOOR_ROUGH:     return JTP_FLOOR_STYLE_ROUGH; 
        case JTP_TILE_FLOOR_ROUGH_LIT: return JTP_FLOOR_STYLE_ROUGH_LIT;
        case JTP_TILE_FLOOR_COBBLESTONE:
        {
            roomid = jtp_get_room_index(floorx, floory);
            switch(roomid % 4)
            {
                case 0: return JTP_FLOOR_STYLE_CERAMIC;
                case 1: return JTP_FLOOR_STYLE_COBBLESTONE;
                case 2: return JTP_FLOOR_STYLE_MOSS_COVERED;
                case 3: return JTP_FLOOR_STYLE_MARBLE;
            }
        }
        case JTP_TILE_FLOOR_WATER:     return JTP_FLOOR_STYLE_WATER;
        case JTP_TILE_FLOOR_ICE:       return JTP_FLOOR_STYLE_ICE;
        case JTP_TILE_FLOOR_AIR:       return JTP_FLOOR_STYLE_AIR;
        case JTP_TILE_FLOOR_LAVA:      return JTP_FLOOR_STYLE_LAVA;
        case JTP_TILE_FLOOR_DARK:      return JTP_FLOOR_STYLE_DARK;
        default:                       return JTP_FLOOR_STYLE_COBBLESTONE; 
    }
}
...
static jtp_tile *jtp_make_transparent_player_tile(int monnum)
{
    ...
    if (monnum != tilenum)
    {
        ...
        tile = (jtp_tile *)malloc(sizeof(*tile));
        if (tile == NULL)
            return montile;
        *tile = *montile;
...
static void jtp_draw_mini_map(void)
{
    ...
    for (i = 0; i < JTP_MAP_HEIGHT; i++)
        for (j = 1; j < JTP_MAP_WIDTH; j++)
        {
            ...
            switch(k)
            {
                case JTP_TILE_WALL_GENERIC:
                case JTP_TILE_UNMAPPED_AREA:
                case JTP_TILE_NONE:
                    l = JTP_COLOR_BACKGROUND; break;
                case JTP_TILE_FLOOR_ROUGH: case JTP_TILE_FLOOR_ROUGH_LIT:
                    l = JTP_COLOR_MINI_CORRIDOR; break;
                case JTP_TILE_STAIRS_UP: case JTP_TILE_STAIRS_DOWN:
                    l = JTP_COLOR_MINI_STAIRS; break;
                case JTP_TILE_VDOOR_WOOD_OPEN: case JTP_TILE_VDOOR_WOOD_CLOSED:
                case JTP_TILE_HDOOR_WOOD_OPEN: case JTP_TILE_HDOOR_WOOD_CLOSED:
                case JTP_TILE_VODBRIDGE: case JTP_TILE_HODBRIDGE: case JTP_TILE_VCDBRIDGE: case JTP_TILE_HCDBRIDGE:
...
void jtp_view_map(void)
{
    ...
    for (i = 0; i < JTP_MAP_HEIGHT; i++)
        for (j = 1; j < JTP_MAP_WIDTH; j++)
        {
            cur_symbol = jtp_back_to_map_symbol(jtp_map_back[i][j]);
            if (jtp_map_tile_is_dark[i][j])
                cur_symbol = -1;
            ...
            if (jtp_map_furniture[i][j] > 0)
            {
                cur_symbol = jtp_furniture_to_map_symbol(jtp_map_furniture[i][j]);
                if (cur_symbol >= 0)
                    jtp_put_img(map_x + 7*j, map_y + 14*i, jtp_map_symbols[cur_symbol]);
            }

            if (jtp_map_obj[i][j] > 0)
                jtp_put_img(map_x + 7*j, map_y + 14*i, jtp_map_symbols[JTP_MAP_SYMBOL_OBJECT]);

            if (jtp_map_trap[i][j] > 0)
                jtp_put_img(map_x + 7*j, map_y + 14*i, jtp_map_symbols[JTP_MAP_SYMBOL_TRAP]);

            if (jtp_map_mon[i][j] > 0)
                jtp_put_img(map_x + 7*j, map_y + 14*i, jtp_map_symbols[JTP_MAP_SYMBOL_MONSTER]);
        }

    /* DEBUG: show center of map window */
    jtp_fill_rect(map_x + 7*jtp_map_x, map_y + 14*jtp_map_y,
                  map_x + 7*jtp_map_x + 6, map_y + 14*jtp_map_y + 13, 15);
...
static int jtp_back_to_map_symbol(int tile)
{
    ...
    switch (tile)
    {
        ...
        case JTP_TILE_FLOOR_COBBLESTONE: case JTP_TILE_FLOOR_ROUGH:
        case JTP_TILE_FLOOR_ROUGH_LIT:
...
static int jtp_furniture_to_map_symbol(int tile)
{
    ...
    switch (tile)
    {
        case JTP_TILE_STAIRS_UP: case JTP_TILE_LADDER_UP:
            cur_symbol = JTP_MAP_SYMBOL_UP; break;
        case JTP_TILE_STAIRS_DOWN: case JTP_TILE_LADDER_DOWN:
            cur_symbol = JTP_MAP_SYMBOL_DOWN; break;
        case JTP_TILE_VDOOR_WOOD_OPEN: case JTP_TILE_VDOOR_WOOD_CLOSED:
        case JTP_TILE_HDOOR_WOOD_OPEN: case JTP_TILE_HDOOR_WOOD_CLOSED:
        case JTP_TILE_VODBRIDGE: case JTP_TILE_HODBRIDGE: case JTP_TILE_VCDBRIDGE: case JTP_TILE_HCDBRIDGE:
            cur_symbol = JTP_MAP_SYMBOL_DOOR; break;
        case JTP_TILE_DOOR_WOOD_BROKEN:
            cur_symbol = JTP_MAP_SYMBOL_FLOOR; break;
        default:
