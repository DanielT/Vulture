...
extern jtp_tilenumber jtp_monster_to_tile(int cur_glyph, XCHAR_P x, XCHAR_P y);
extern jtp_tilenumber jtp_object_to_tile(int obj_id);
extern void        jtp_clear_walls(int y, int x);
extern unsigned char *jtp_choose_target_tooltip(int tgt_x, int tgt_y);
extern int jtp_get_mouse_inventory_input(jtp_mouse_cursor * m_cursor, jtp_hotspot ** hotspots, int n_hotspots, int whenstop);
