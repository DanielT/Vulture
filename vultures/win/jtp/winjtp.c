...
int glyph;
{
  ...
  void drawBackgroundTile() {
    k = glyph_to_cmap(back_to_glyph(x,y));
  ...
  /* save any special attributes for this tile */
  mapglyph(glyph,(int *)&character,(int *)&colour,(unsigned int *)&jtp_mapglyph_special[y][x],x,y);
  ...
  if (glyph_is_cmap(glyph))
  {
    ...
    /* TODO use clive's passages patches do deal with this sort of thing more
     * naturally
     */
    if (k == S_stone)
    {
...
/*    
    printf("%d -> %d: [%d, %d] (Wanted: %d)\n",glyph, k, x, y, SACK );

    if ((!known) && ((glyph == LUCKSTONE) || (glyph == LOADSTONE) || (glyph == FLINT)) )
    { //Special case: Unidentified stone
        k = ROCK;
    }

    if ((!known) && ((glyph == SAPPHIRE) || (glyph == TURQUOISE)) ))
    { //Special case: Unidentified gem
        k = BLUE_GLASS;
    }

    if ((!known) && ((glyph == RUBY) || (glyph == GARNET) || (glyph == JASPER)) )
    { //Special case: Unidentified gem
        k = RED_GLASS;
    }

    if ((!known) && ((glyph == CITRINE) || (glyph == CHRYSOBERYL)) )
    { //Special case: Unidentified gem
        k = YELLOW_GLASS;
    }

    if ((!known) && ((glyph == AMETHYST) || (glyph == FLUORITE)) )
    { //Special case: Unidentified gem
        k = VIOLET_GLASS;
    }

    if ((!known) && ((glyph == EMERALD) || (glyph == AQUAMARINE) || (glyph == JADE)) )
    { //Special case: Unidentified gem
        k = GREEN_GLASS;
    }

    if ((!known) && ((glyph == BLACK_OPAL) || (glyph == JET) || (glyph == OBSIDIAN)) )
    { //Special case: Unidentified gem
        k = BLACK_GLASS;
    }

    if ((!known) && ((glyph == DILITHIUM_CRYSTAL) || (glyph == DIAMOND) || (glyph == OPAL)) )
    { //Special case: Unidentified gem
        k = WHITE_GLASS;
    }

    if ((!known) && ((glyph == AMBER) || (glyph == TOPAZ)) )
    { //Special case: Unidentified gem
        k = ORANGE_GLASS;
    }

    if ((!known) && ((glyph == SACK) || (glyph == BAG_OF_HOLDING) || (glyph == OILSKIN_SACK) || (glyph == BAG_OF_TRICKS) ) )
    { //Special case: Unidentified bag
        k = SACK;
    }
*/
