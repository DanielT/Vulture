...
static void drawBackgroundTile(XCHAR_P x, XCHAR_P y)
{
	int k;
...
void jtp_print_glyph(winid window, XCHAR_P x, XCHAR_P y, int glyph)
{
  ...
  mapglyph(glyph, &character, &colour, &jtp_mapglyph_special[y][x], x, y);
  ...
  switch(tempwindow->wintype)
  {
    ...
    case NHW_MAP:
      if ((jtp_map_changed) && (!jtp_map_is_empty))
      {
        ...
        if ((!jtp_map_is_empty) && (!jtp_is_onscreen(u.ux, u.uy)))
        {
          jtp_map_x = u.ux;
          jtp_map_y = u.uy;
        }
